...
public class AssassinTask implements Task {
  ...
  private void sendInfo(long chatId) {
    ...
    botActionCollector.text(
        chatId,
        new SimpleText(
            "Однако не всё так просто!\nЕсли закажешь кого-либо, вот что может произойти:"
                + "\n\n-50% что ничего не произойдёт"
                + "\n\n-40% что указанный тобою человек станет пидором дня"
                + "\n\n-10% что ты сам окажешься пидором"));
    botActionCollector.wait(chatId, ChatAction.TYPING);
    botActionCollector.text(
        chatId,
        new ParametizedText(
